...
class ViewController: UIViewController,
  ...
  func applyNumKartsSetting(_ settings: [String : Any]) {
	  guard let numKarts = settings["numKarts"] as? Int else {
		  assertionFailure("Expected Int, but got nil")
		  return
	  }
  ...
  func applyKartSizeSetting(_ settings: [String : Any]) {
	  guard let kartSizeMulti = settings["kartSize"] as? Int else {
		  assertionFailure("Expected Int, but got nil")
		  return
	  }
	  let kartSize = 1.0 + 0.05 *  Double(kartSizeMulti)
	  print(kartSize)
  ...
  func applySpeedMultiplierSetting(_ settings: [String : Any]) {
	  guard let speedMulti = settings["speedMultiplier"] as? Int else {
		  assertionFailure("Expected Int, but got nil")
		  return
	  }
	  self.speedMultiplier = Double(speedMulti)
  }
}
